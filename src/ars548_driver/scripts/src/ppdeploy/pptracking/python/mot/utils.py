...
def flow_statistic_multi_class(result,
                   secs_interval,
    ...
    if do_entrance_counting and len(result[1]) > 0:
        # assert region_type in [
        #     'horizontal', 'horizontal-C','horizontal-B', 'horizontal-A', 'vertical',
        # ], "region_type should be 'horizontal' or 'vertical' when do entrance counting."
        entrance_x, entrance_y = entrance[0], entrance[1]
        frame_id, mot_result = result
        tlwhs = mot_result[:, 3:]
        tscores = mot_result[:, 2]
        track_ids = [int(i) for i in mot_result[:, 0]]
        cls_ids = [int(i) for i in mot_result[:, 1]]
        # print("$"*10)
        # print(result)
        # print("$"*10)
        for tlwh, score, track_id, cls_id in zip(tlwhs, tscores, track_ids, cls_ids):
            if track_id < 0: continue
            if data_type == 'kitti':
                frame_id -= 1
            # x1, y1, w, h = tlwh
            ...
            center_y = (y2-y1)/2.0+y1
            if cls_id == 0 and track_id == 2:
                print('---')
                if cls_id in prev_center and track_id in prev_center[cls_id]:
                    print((prev_center[cls_id][track_id]))
                print((center_x, center_y))
                print('---')
            if cls_id in prev_center and track_id in prev_center[cls_id]:
                if region_type == 'horizontal' or region_type == 'horizontal-A' or region_type == 'horizontal-B' or region_type == 'horizontal-C' or 'tunnel' in region_type:
                    # horizontal center line
                    if prev_center[cls_id][track_id][1] <= entrance_y and \
                    center_y > entrance_y:
                        in_id_list.append(f'{cls_id}_{track_id}')
                    if prev_center[cls_id][track_id][1] >= entrance_y and \
                    center_y < entrance_y:
                        out_id_list.append(f'{cls_id}_{track_id}')
                else:
                    # vertical center line
                    if prev_center[cls_id][track_id][0] <= entrance_x and \
                    center_x > entrance_x:
                        in_id_list.append(f'{cls_id}_{track_id}')
                    if prev_center[cls_id][track_id][0] >= entrance_x and \
                    center_x < entrance_x:
                        out_id_list.append(f'{cls_id}_{track_id}')
                prev_center[cls_id][track_id][0] = center_x
                prev_center[cls_id][track_id][1] = center_y
            else:
                if not cls_id in prev_center:
                    prev_center[cls_id] = defaultdict()
                prev_center[cls_id][track_id] = [center_x, center_y]
        ...
        for tlwh, score, track_id in zip(tlwhs, tscores, track_ids):
            ...
            if frame_id == 1:
                ...
                if track_id in prev_center:
                    if not in_quadrangle(prev_center[cls_id][track_id], entrance, im_h,
    ...
    if len(result[1]) > 0:
        ...
        for tlwh, score, track_id, cls_id in zip(tlwhs, tscores, track_ids, cls_ids):
            if track_id < 0: continue
            id_set.add(f'{cls_id}_{track_id}')
            interval_id_set.add(f'{cls_id}_{track_id}')
    ...
    if reset and frame_id % video_fps == 0 and frame_id / video_fps % secs_interval == 0:
        curr_interval_count = len(interval_id_set)
        interval_id_set.clear()
    info = "Frame id: {}, Interval count: {}".format(frame_id, len(in_id_list)+len(out_id_list))
    

    # disable due to incapable with multiclass MOT
    ...
    return {
        ...
        "records": records,
        "current_interval_count": curr_interval_count
    }
