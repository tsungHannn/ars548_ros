...
def plot_tracking_dict(image,
    ...
    if num_classes == 1:
        if records is not None:
            ...
            cv2.putText(
                im,
                records[-1][start:end], (20, int(10 * text_scale) + 10),
                cv2.FONT_ITALIC,
                text_scale, (0, 0, 255),
                thickness=text_thickness)
    ...
    if num_classes == 1 and do_entrance_counting:
        ...
        text_size, _ = cv2.getTextSize(records[-1][start:-1], cv2.FONT_ITALIC, text_scale, text_thickness)
        text_w, text_h = text_size
        cv2.rectangle(im, (20, int(20 * text_scale) - 20), (20 + text_w, int(20 * text_scale) + 20 + text_h), (0, 0, 0), -1)
        cv2.putText(
            im,
            records[-1][start:-1], (20, int(20 * text_scale) + 20),
            cv2.FONT_ITALIC,
            text_scale, (0, 0, 255),
            thickness=text_thickness)
    ...
    if multi_class_dict:
        num_classes = multi_class_dict["num_classes"]
        tlwhs_dict = defaultdict(list)
        obj_ids_dict = defaultdict(list)
        scores_dict = defaultdict(list)
        for box in multi_class_dict["mot_res"]:
            cls_id = int(box[1])
            obj_id = int(box[0])
            score = box[2]
            tlwh = box[3:]
            ...
            tlwhs_dict[cls_id].append(tlwh)
            obj_ids_dict[cls_id].append(obj_id)
